describe('Planet flight reservation user flows', () => {
  ...
  it.skip('should have carousel to navigate between landing pages', () => {
    cy.get('[class*=mySwiper]')
    .children()
    .should('be.visible')
  })

  it.skip('should be able to click on the swiper buttons to scan through the landing pages', () => {
    cy.get('[class^=swiper-button-next]')
    .click( {multiple: true} )
  })

  it.skip('should be able to click on a planet and display the name of the destination', () => {
    cy.get('[class*=mySwiper]')
    .children()
    .contains('lune')
    .click()
